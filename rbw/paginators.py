...
class PaginatorEmbedInterface(PaginatorInterface):
    ...
    def send_kwargs(self) -> dict:
        display_page = self.display_page
        self._embed.title = f"{self.bot.user.name}'s Help Command"
        self._embed.description = self.pages[display_page]
        self._embed.color = 0xF41008 # CHANGE THIS TO THE COLOR YOU WANT
        self._embed.set_footer(text=f'Page {display_page + 1}/{self.page_count} | Bot made by Nuggets#0007 and GozZzer#1245. Problems? DM us!')
